...
public class BlockInfo extends Module {
    ...
    public void onRender() {
        ...
        fr.drawStringWithShadow(blockName,
                (float) ((sr.getScaledWidth_double() / 2 - fr.getStringWidth(blockName) / 2) + yTextOffset),
                4,
                0xFFFFFFFF);
        ...
        if (block == Blocks.bedrock ||
            ...
            fr.drawStringWithShadow("\u2717 Unbreakable",
                    (float) ((sr.getScaledWidth_double() / 2 - fr.getStringWidth("\u2717 Unbreakable") / 2) + yTextOffset),
                    (fr.FONT_HEIGHT * 2) - 6,
                    0xFFFF0000);
            return;
        }
        ...
        if (canHarvest) fr.drawStringWithShadow("\u2713 Currently Harvestable",
                (float) ((sr.getScaledWidth_double() / 2 - fr.getStringWidth("\u2713  Currently Harvestable") / 2) + yTextOffset),
                (fr.FONT_HEIGHT * 2) - 6,
                0xFF00FF00);
        ...
        fr.drawStringWithShadow("Effective Tool: " + harvestTool,
                (float) ((sr.getScaledWidth_double() / 2 - fr.getStringWidth("Effective Tool: " + harvestTool) / 2) + yTextOffset),
                (fr.FONT_HEIGHT * 3) - 6,
                hasEffectiveTool ? 0xFF00FF00 : 0xFFFF0000);

        // draw harvest level
        fr.drawStringWithShadow("Harvest Level: " + harvestLevelName,
                (float) ((sr.getScaledWidth_double() / 2 - fr.getStringWidth("Harvest Level: " + harvestLevelName) / 2) + yTextOffset),
                (fr.FONT_HEIGHT * 4) - 6,
        ...
        mc.getRenderItem().renderItemAndEffectIntoGUI(new ItemStack(Item.getItemFromBlock(iblockstate.getBlock())),
                (int) (((sr.getScaledWidth_double() / 2) - (100 - yTextOffset)) / 2),
                2);
